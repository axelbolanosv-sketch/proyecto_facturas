...
def render_kpi_dashboard(lang, resultado_df):
    ...
    col2.metric(label=get_text(lang, 'kpi_total_amount'), value=f"${totales_numericos.sum():,.2f}", help=get_text(lang, 'kpi_total_amount_help'))
    col3.metric(label=get_text(lang, 'kpi_avg_amount'), value=f"${totales_numericos.mean():,.2f}" if not totales_numericos.empty else "$0.00", help=get_text(lang, 'kpi_avg_amount_help'))
...
def render_detailed_view(lang, resultado_df_filtrado, df_master_copy, col_map_ui_to_en, todas_las_columnas_en):
    ...
    if 'Priority' in resultado_df_filtrado.columns:
        try:
            ...
            new_index = np.where(is_max_priority, "🚩 " + df_display.index.astype(str), df_display.index.astype(str))
    ...
    if 'editor_state' not in st.session_state or st.session_state.current_data_hash != current_data_hash or st.session_state.current_lang_hash != current_lang_hash:
    ...
    def callback_add_row():
        ...
        if not df_editado.empty:
            clean_indices = pd.to_numeric(pd.Series(df_editado.index).astype(str).str.replace("🚩 ", ""), errors='coerce').dropna()
            if not clean_indices.empty: max_index = max(max_index, clean_indices.max())
        new_index = int(max_index + 1) 
        default_values = {}
        for col in df_editado.columns:
            if col == col_sel_name: default_values[col] = False; continue
            ...
            if pd.api.types.is_numeric_dtype(col_original_dtype): default_values[col] = 0
            else: default_values[col] = ""
    ...
    def _callback_guardar_borrador():
        try:
            ...
            if col_sel_name in df_edited_view_ui.columns: df_edited_view_ui = df_edited_view_ui.drop(columns=[col_sel_name])

            df_edited_view_ui.index = pd.to_numeric(df_edited_view_ui.index.astype(str).str.replace("🚩 ", ""))
            ...
            st.session_state.editor_state = None; st.session_state.current_data_hash = None; st.session_state.current_lang_hash = None
    ...
    def _callback_guardar_borrador_and_rerun(): _callback_guardar_borrador(); st.rerun()

    def _callback_guardar_estable():
        st.session_state.df_original = st.session_state.df_staging.copy()
        if st.session_state.columnas_visibles is not None: st.session_state.columnas_visibles_estable = st.session_state.columnas_visibles.copy()
        st.success(get_text(lang, 'commit_success_message'))

    def _callback_revertir_estable():
        if st.session_state.df_original is not None: st.session_state.df_staging = st.session_state.df_original.copy()
        if st.session_state.columnas_visibles_estable is not None: st.session_state.columnas_visibles = st.session_state.columnas_visibles_estable.copy()
        st.session_state.editor_state = None; st.session_state.current_data_hash = None; st.session_state.current_lang_hash = None
    ...
    with col1: st.button(get_text(lang, 'add_row_button'), on_click=callback_add_row, use_container_width=True, help=get_text(lang, 'add_row_help'))
    with col2: st.button(get_text(lang, 'save_changes_button'), on_click=_callback_guardar_borrador_and_rerun, use_container_width=True, help=get_text(lang, 'save_changes_help'), type="primary")
    with col3: st.button(get_text(lang, 'commit_changes_button'), on_click=_callback_guardar_estable, use_container_width=True, help=get_text(lang, 'commit_changes_help'), key="commit_changes")
    with col4: st.button(get_text(lang, 'reset_changes_button'), on_click=_callback_revertir_estable, use_container_width=True, help=get_text(lang, 'reset_changes_help'), key="reset_changes_button")
    st.warning(get_text(lang, 'editor_info_help_add_row'))

    # --- HOTKEYS ---
    if hotkeys.pressed("revert_stable", key='main_hotkeys'): _callback_revertir_estable(); st.rerun() 
    elif hotkeys.pressed("add_row", key='main_hotkeys'): callback_add_row(); st.rerun() 
    elif hotkeys.pressed("save_stable", key='main_hotkeys'): _callback_guardar_estable() 
    elif hotkeys.pressed("save_draft", key='main_hotkeys'): _callback_guardar_borrador(); st.rerun() 
    ...
    with col_dl1:
        ...
        if col_sel_name in df_dl.columns: df_dl = df_dl.drop(columns=[col_sel_name])
        ...
        st.download_button(get_text(lang, 'download_excel_manual_edits_button'), to_excel(df_dl), "resultado_facturas_BORRADOR.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="download_excel_editado", use_container_width=True)
    with col_dl2:
        st.download_button(get_text(lang, 'download_excel_filtered_button'), to_excel(df_vista_detallada), "resultados_Filtrados.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="download_excel_filtrado", use_container_width=True)
    with col_restore:
        if st.button(get_text(lang, 'restore_pristine_button'), use_container_width=True, help=get_text(lang, 'restore_pristine_help'), key="restore_pristine"):
            if st.session_state.df_pristine is not None:
                st.session_state.df_original = st.session_state.df_pristine.copy(); st.session_state.df_staging = st.session_state.df_pristine.copy()
                st.session_state.columnas_visibles = list(st.session_state.df_pristine.columns); st.session_state.columnas_visibles_estable = list(st.session_state.df_pristine.columns)
            st.session_state.editor_state = None; st.session_state.current_data_hash = None; st.session_state.current_lang_hash = None; st.rerun() 
...
def render_grouped_view(lang, resultado_df, col_map_ui_to_en, todas_las_columnas_en):
    ...
    col_para_agrupari = st.selectbox(get_text(lang, 'group_by_select'), options=opciones_agrupables_ui, key='group_by_col_select')
    ...
    if col_para_agrupari:
        ...
        if 'Total' in df_agg.columns: df_agg['Total'] = pd.to_numeric(df_agg['Total'], errors='coerce')
        if 'Invoice Date Age' in df_agg.columns: df_agg['Invoice Date Age'] = pd.to_numeric(df_agg['Invoice Date Age'], errors='coerce')

        agg_ops = {'Total': ['sum', 'mean', 'min', 'max', 'count']}
        if 'Invoice Date Age' in df_agg.columns and pd.api.types.is_numeric_dtype(df_agg['Invoice Date Age']): agg_ops['Invoice Date Age'] = ['mean']

        try: 
            df_res = df_agg.groupby(col_en).agg(agg_ops)
            cols_names = [get_text(lang, 'group_total_amount'), get_text(lang, 'group_avg_amount'), get_text(lang, 'group_min_amount'), get_text(lang, 'group_max_amount'), get_text(lang, 'group_invoice_count')]
            if 'Invoice Date Age' in agg_ops: cols_names.append(get_text(lang, 'group_avg_age'))
            df_res.columns = cols_names
            
            st.dataframe(df_res.sort_values(by=get_text(lang, 'group_total_amount'), ascending=False))
            st.download_button(get_text(lang, 'download_excel_button'), to_excel(df_res), f"agrupado_por_{col_para_agrupari}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="download_excel_agrupado")
        except Exception as e: st.error(f"Error: {e}")
    else: st.info("Seleccione una columna.")